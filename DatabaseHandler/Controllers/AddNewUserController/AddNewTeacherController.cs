...
namespace DatabaseHandler.Controllers.AddNewUserController
{
    ...
    public class AddNewTeacherController : BaseController
    {
        ...
        [HttpPost]
        public async Task<IActionResult> NewTeacher([FromBody] HashSet<Visible_teacher> newUsers)
        {
            try
            {
                _newUsers = newUsers;
                foreach(var _newUser in _newUsers)
                {
                    ...
                    await _addNewTeacher.user.Add(_user);
                    _teacher.Individual = _user;

                    Department dept = await _deptRepository.getDeptByName(_newUser.Dept);
                    _teacher.Dept = dept;
                    _teacher.Designation = _newUser.Designation;

                    await _addNewTeacher.SpecificTypeUser.Add(_teacher);
                }

                
                await _addNewTeacher.Complete();
