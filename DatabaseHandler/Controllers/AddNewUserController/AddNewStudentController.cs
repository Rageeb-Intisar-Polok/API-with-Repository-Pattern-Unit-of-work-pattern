...
namespace DatabaseHandler.Controllers.AddNewUserController
{
    ...
    public class AddNewStudentController : BaseController
    {
        ...
        [HttpPost]
        public async Task<IActionResult> NewOfficial([FromBody] HashSet<Visible_student> newUsers)
        {
            try
            {
                _newUsers = newUsers;
                foreach (var _newUser in _newUsers)
                {
                    ...
                    Department department = await _deptRepository.getDeptByName(_newUser.Dept);

                    int level = _newUser.level;
                    string term = _newUser.term;

                    Level_Term foundLt = await _levelTermRepository.getLevelTermEntity(level, term);
                    student.LT = foundLt;
                    student.Dept = department;
                    await _addNewStudent.user.Add(user);
                    ...
                    await _addNewStudent.SpecificTypeUser.Add(student);
                }
                

                await _addNewStudent.Complete();
