...
namespace DatabaseHandler.Controllers.AddNewUserController
{
    ...
    public class AddNewOfficialController : BaseController
    {
        ...
        [HttpPost]
        public async Task<IActionResult> NewOfficial([FromBody] HashSet<Visible_official> newUsers)
        {
            try
            {
                _newUsers = newUsers;
                ...
                {
                    Role role = await _roleRepository.getByName(_newUser.Role);
                    ...
                    await _addNewOfficial.user.Add(_baseInfo);


                    _otherInfo.Individual = _baseInfo;
                    await _addNewOfficial.SpecificTypeUser.Add(_otherInfo);
                }


                await _addNewOfficial.Complete();
