...
namespace DatabaseHandler.Controllers.AboutUser.Works.AboutStudent
{
    public class GetInfoAboutStudentWork : IGetInfoAboutStudentWork
    {
        ...
        public async Task<Visible_student> getVisibleStudent(int individualId)
        {
            int id = individualId;
            Students student = await _studentRepository.GetStudentByUniqueId(id);


            Visible_student visible_Student = new Visible_student();
            Department dept;
            int deptId = student.DeptId;
            dept = await _deptRepository.getDepartmentById(deptId);
            string a = dept.DeptName;
            visible_Student.Dept = a;
            int levelId = student.LTId;
            Level_Term lt = await _levelTermRepository.getLevelTermById(levelId);
